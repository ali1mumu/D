...
class DualPoseNet(object):
    ...
    def allocate_placeholders(self):
        ...
        self.input_dis = tf.placeholder(tf.float32, shape=[
                                        self.opts.batch_size, self.opts.input_res, self.opts.input_res, 1]) # [64,64,64,1]
        self.input_rgb = tf.placeholder(tf.float32, shape=[
                                        self.opts.batch_size, self.opts.input_res, self.opts.input_res, 3]) # [64,64,64,3]
        self.observed_pc = tf.placeholder(
            tf.float32, shape=[self.opts.batch_size, 1024, 3])  # 64,1024,3

        self.gt_rotation = tf.placeholder(
            tf.float32, shape=[self.opts.batch_size, 4])    # 64，4
        self.gt_translation = tf.placeholder(
            tf.float32, shape=[self.opts.batch_size, 3])    # 64，3
    ...
    def train(self):
        ...
        # dataset
        print('loading data ...')
        fetchworker = Fetcher(self.opts)  # datasets 19193
        fetchworker.start()
        print('data loaded !')

        print('starting training ...')
        self.sess.run(tf.global_variables_initializer())  # 初始化模型参数
        ...
        for epoch in range(1, self.opts.training_epoch+1):
            ...
            for batch_idx in range(fetchworker.num_batches):
                ...
                #  [64,64,64,1]   [64,64,64,3]      [64,1024,3]         [64,4]            [64,3]            [64,3]
